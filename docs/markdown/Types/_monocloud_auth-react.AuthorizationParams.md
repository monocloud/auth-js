...
| `authenticatorHint?`     | [`Authenticators`](/sdks/react/api-reference/enums/authenticators)           | Hint to the authorization server indicating which authenticator or connection should be used.                                                                            |
| `codeChallenge?`             | `string`                                                                                 | PKCE code challenge derived from the code verifier. Used to secure authorization code exchanges.                                                                         |
| `codeChallengeMethod?` | [`CodeChallengeMethod`](/sdks/react/api-reference/enums/codechallengemethod) | Method used to generate the PKCE code challenge.                                                                                                                         |
...
| `prompt?`                           | [`Prompt`](/sdks/react/api-reference/enums/prompt)                           | Controls authentication interaction behavior. For example, forcing login or consent.                                                                                     |
...
| `responseMode?`               | [`ResponseModes`](/sdks/react/api-reference/enums/responsemodes)             | Specifies how the authorization response is returned to the client.                                                                                                      |
| `responseType?`               | [`ResponseTypes`](/sdks/react/api-reference/enums/responsetypes)             | Determines which artifacts are returned from the authorization endpoint.                                                                                                 |
