...
| `idTokenClockTolerance?` | `number`                                                                                                 | Additional allowed clock tolerance (in seconds) when validating time-based ID token claims (`exp` and `nbf`) and the elapsed-authentication (`max_age`) check.    |
| `jwks?`                                   | [`Jwks`](/sdks/nodejs-core/api-reference/types/jwks)                                                        | JSON Web Key Set used to validate the ID token signature. If not provided, the JWKS is automatically fetched from the authorization server metadata.              |
...
| `refreshGrantOptions?`     | [`RefreshGrantOptions`](/sdks/nodejs-core/api-reference/types/refreshgrantoptions)                          | Options applied to the refresh token grant request, such as requesting tokens for specific resources or scopes.                                                   |
