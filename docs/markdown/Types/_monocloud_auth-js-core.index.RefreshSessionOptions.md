...
| `jwks?`                                   | [`Jwks`](/sdks/js-core/api-reference/types/jwks)                                             | JSON Web Key Set used to validate the ID token signature. If not provided, the JWKS is automatically fetched from the authorization server metadata.              |
...
| `refreshGrantOptions?`     | [`RefreshGrantOptions`](/sdks/js-core/api-reference/types/refreshgrantoptions)               | Options applied to the refresh token grant request, such as requesting tokens for specific resources or scopes.                                                   |
