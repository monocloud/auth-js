...
- The transactions which can no longer be completed are removed after a successful callback, leaving the sign ins in progress in other tabs untouched. Signing out discards all of them, since the session it would restore is shared by every tab. The number of concurrent transactions retained is configurable through `state.maxConcurrent` or `MONOCLOUD_AUTH_STATE_MAX_CONCURRENT` (`1`–`20`, default `5`), with the earliest ones evicted first. Set it to `1` for sequential sign ins, where starting a new sign in discards the previous pending one.
