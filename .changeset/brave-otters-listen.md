...
- `validateCertificateBinding` is now a client option (`MONOCLOUD_BACKEND_VALIDATE_CERTIFICATE_BINDING`) taking `'when_present'` (default), `'required'`, or `'dangerously_ignore'`, and is removed from `ProtectOptions`; tokens whose `cnf` claim carries an `x5t#S256` thumbprint are now validated by default.
