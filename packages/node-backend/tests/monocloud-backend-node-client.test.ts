...
// Note: `delete` is included in cache fixtures to satisfy the ICache interface;
// the client itself only invokes `get` and `set`.
  ...
  describe('validateAccessToken', () => {
    ...
    describe('caching', () => {
      ...
      it('should return cached claims when present and not expired', async () => {
        ...
        expect(cache.get).toHaveBeenCalledWith(jwtToken);
        expect(jwtSpy).not.toHaveBeenCalled();
        expect(introspectSpy).not.toHaveBeenCalled();
      });
      ...
      it('should not store claims in cache when the result has no exp', async () => {
        ...
        vi.spyOn(client, 'validateJwtAccessToken').mockResolvedValue(fresh);
      ...
      it('should store freshly-validated claims in the cache', async () => {
        ...
        expect(cache.set).toHaveBeenCalledWith(jwtToken, fresh, fresh.exp);
      });
      ...
      it('should propagate errors thrown by cache.get', async () => {
        ...
        await expect(client.validateAccessToken(jwtToken)).rejects.toThrow(
          'cache offline'
        );
        expect(jwtSpy).not.toHaveBeenCalled();
        expect(cache.set).not.toHaveBeenCalled();
